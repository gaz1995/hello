...
investment_horizon = st.sidebar.slider('Which is your investment horizon (the minimum inv.horizon is 10 years)?',
                                       value=20, min_value=10, max_value=40)
monthly_contribution = st.sidebar.number_input(value=1000, label="What is your monthly contribution in dollar amount?",
                                               min_value=0, max_value=1000000, step=100)
inflation = st.sidebar.selectbox(label="What is your annual inflation expectation?",
                                 options=[0, 0.01, 0.015, 0.02, 0.025, 0.03])
initial_investment = st.sidebar.number_input(value=10000,
                                             label="What is your initial investment amount in dollar amount?",
...
sp.add_patch(re2)
plt.title(
    f"Our strategy's total return was {int(returns_df['strategy_csum'].max() * 100)}% while for full equity was {int(returns_df['full_equity_csum'].max() * 100)}% and for the 60/40 was {int(returns_df['sixty_forty_csum'].max() * 100)}% during this period.",
    size=13)
plt.xlabel("Years")
...
st.write("Drawdown is very useful at verifiying the peak-to-trough. Are you able to stomach a drop of more than 50%?")
# Plot drawdowns
...
sp.add_patch(re2)
plt.title(
    f"Our strategy's maximum drawdown was {int(max_drawdown(returns_df['strategy']).min())}% while for full equity was {int(max_drawdown(returns_df['full_equity_returns']).min())}% and for 60/40 was {int(max_drawdown(returns_df['sixty_forty_returns']).min())}%.",
    size=13)
plt.xlabel("Years")
...
proposed_annual_return = summary_table["Our Strategy"]["Annual Return"]
proposed_annual_volatility = summary_table["Our Strategy"]["Annual Volatility"]
...
future_expectations["upper_ci"] = performance(returns_df["strategy"])["Annual Return"] + stats.norm.ppf(0.95) * \
                                  performance(returns_df["strategy"])["Annual Volatility"] / np.sqrt(252)
future_expectations["lower_ci"] = performance(returns_df["strategy"])["Annual Return"] - stats.norm.ppf(0.95) * \
                                  performance(returns_df["strategy"])["Annual Volatility"] / np.sqrt(252)
...
st.markdown(
    "***Now we consider the future time horizon. Here, everything is an expectation and nothing is guaranteed***")
...
ax.fill_between(future_expectations.index, future_expectations["upper_ci_csum"] * 100,
                future_expectations["lower_ci_csum"] * 100, alpha=0.2)
plt.title(
    f"The expected total returns fall between {round(future_expectations.iloc[investment_horizon, 5] * 100, 2)}% and {round(future_expectations.iloc[investment_horizon, 4] * 100, 2)}% in the span of {investment_horizon} years, considering our past performance.",
    size=13)
plt.xlabel("Years")
...
st.write(
    "We must consider the effect of inflation on the portfolio's returns. Staying in cash is a bad idea if you expect positive inflation.")
...
surpass = future_expectations.index[
    future_expectations["capital_gains_csum"] > future_expectations["contributions_csum"]].min()
...
st.write(
    "What do you value in a portfolio? Consistency, high returns, downside protection? Verify if these annual results satisfy you.")
...
for year in grouped_df["year"]:
    if grouped_df["strategy"][year] > grouped_df["full_equity_returns"][year] and grouped_df["strategy"][year] > \
            grouped_df["sixty_forty_returns"][year]:
        outperform_count += 1
...
plt.title(
    f"Our strategy outperforms {outperform_count} out of the {(dt.date.today()).year - 2007} past years in terms of returns.")
plt.grid(axis="y", linestyle="--")
plt.ylim(-0.5, 0.5)
ax.set_axisbelow(True)
ax.tick_params(labeltop=False, labelright=True)
...
last_weights.plot(x="year", kind="bar", ax=ax,
                  color=['tab:blue', 'tab:red', 'seagreen', 'tab:orange', 'tab:purple', 'teal', 'midnightblue', 'gold'])
...
plt.title(
    f"Since we follow risk parity, most of the portfolio will be allocated to safer assets. That is why bonds make for {round(last_weights[['US_TREASURIES', 'TIPS', 'CORP_BONDS']].sum())}% of the portfolio")
ax.tick_params(labeltop=False, labelright=True)
plt.grid(axis="y", linestyle="--")
ax.set_axisbelow(True);
st.pyplot(fig7)
...
st.write(
    "This matrix allows us to verify how each assets behaves in relation to the others. Low and negative correlations are key for diversification purposes!")
...
sns.heatmap(correlation_df, annot=True, cmap="viridis",
            cbar_kws={"label": "Correlation Intensity: 1 =Perfectly Correlated and -1 =Inversly Correlated"})
plt.ylabel("Asset Classes")
plt.title(
    "Correlation between the assets proves what we already know: bonds and stocks have low to negative correlation.",
    pad=20);
st.pyplot(fig8)
